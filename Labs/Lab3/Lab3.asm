...
methodLoop1_spaces:
    	bgtz t2, methodPrinting_space   # space print
    	j methodPrinting_star
    	write_to_buffer(0x2a) 
             
methodPrinting_space:
...
methodCheck_int2: 
	bgt t1, t3, methodLoop3_exit
	sub t4, t3, t1
	
methodLoop2_spaces: 
        beqz t4, methodEnd_loop   # exit
        print_str(blankspace)
        write_to_buffer(0x20)
        addi t4, t4, -1
        j methodLoop2_spaces   # repeat

methodEnd_loop: #exit the loop
...
methodLoop3_spaces: 
	beqz t5, methodEnding_loop2	
	print_str(blankspace)
	write_to_buffer(0x20)
	addi t5, t5, -1
	j methodLoop3_spaces

methodEnding_loop2: #exits
	print_str(star)
	write_to_buffer(0x2a) 
	print_str(newLine)
	write_to_buffer(0x0a)
